...
<div class="container" id="container">
    <div  id="header">
		...
		<div id="content_main" class="clearfix">
			...
			<div class="section">
			<h2 class="ico_mug">Shipping Records</h2>
		<table id="table">
			<thead>
			<tr>
            ...
            <?php
			$per_page = 20;
			$pages_query = mysql_query("SELECT COUNT('email') from funds where amount >= 50") or die(mysql_error());
			$pages = ceil(mysql_result($pages_query,0) / $per_page);
			
			$page = mysql_real_escape_string((isset($_GET['page'])) ? (int)$_GET['page'] : 1);
			$start = ($page - 1) * $per_page;
			
			$query = mysql_query("select * from funds where amount >= 50 ORDER BY id DESC LIMIT $start, $per_page") or die(mysql_error());
			while ($row = mysql_fetch_assoc($query)){
				...
				echo "<tr>
				<td class='table_check'><input type='checkbox' class='noborder' name='id[]' value='$id' /></td>
				<td class='table_date'>$first_name</td>
				<td class='table_date'>$email</td>
				<td><a href='view-shiping-records.php?txn=$transaction_id'><img src='img/folder.jpg' alt='folder'/></a></td>";
				if ($status=="qued"){
					echo "<td><span class='qued'>Qued</span></td>";
				}else if ($status=="process"){
					echo "<td><span class='process'>Processing</span></td>";
				}else{
					echo "<td><span class='approved'>Completed</span></td>";
				}
			echo "</tr>";
			}
			?>
		...
		<div id="table_options" class="clearfix">
				...
				<ul>
					...
					<li><label>	Action: &nbsp; <select id="kategoria" name="kategoria">
									...
									<option value="2">View Full Info</option> 
									<option value="2">Delete</option> 
								</select>
				</label></li>
				<li><label>	Status: &nbsp; <select id="status_selector" name="status_selector">
									<option value="1">Select one</option> 
