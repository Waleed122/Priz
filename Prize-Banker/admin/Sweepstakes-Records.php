...
if ($_GET['delete']){
	...
	$query = mysql_query("delete from sweepstakes where id='$id'") or die(mysql_error());
}

if ($_POST['delete']){
	$id = $_POST['delete'];
	$ids = Protect(implode(",",$id));
	$query = mysql_query("delete from sweepstakes where id in ($ids)") or die(mysql_error());
}
?>
...
<div class="container" id="container">
    <div  id="header">
		...
		<div id="content_main" class="clearfix">
			...
			<div class="section">
			<h2 class="ico_mug">Sweepstakes Email Records</h2>
		<table id="table">
			<thead>
			<tr>
            ...
            <?php
			$per_page = 20;
			$pages_query = mysql_query("SELECT COUNT('email') from sweepstakes") or die(mysql_error());
			$pages = ceil(mysql_result($pages_query,0) / $per_page);
			
			$page = mysql_real_escape_string((isset($_GET['page'])) ? (int)$_GET['page'] : 1);
			$start = ($page - 1) * $per_page;
			
			$query = mysql_query("select * from sweepstakes ORDER BY id DESC LIMIT $start, $per_page") or die(mysql_error());
			while ($row = mysql_fetch_assoc($query)){
				...
				echo "<tr>
				<td class='table_check'><input type='checkbox' class='noborder' name='delete[]' value='$id' /></td>
				<td class='table_date'>$name</td>
				<td class='table_date'>$email</td>
				<td><a href='Edit-Testimonials.php?id=$id'><img src='img/edit.jpg' alt='edit'/></a><a href='?delete=$id' class='delete' id='$id'><img src='img/cancel.jpg' alt='cancel'/></a></td>
			</tr>";
			}
			?>
            </form>
